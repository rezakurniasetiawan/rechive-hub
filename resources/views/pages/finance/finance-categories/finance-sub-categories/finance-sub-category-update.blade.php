...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 lg:col-span-6">
        ...
        <div class="intro-y box p-5">
            <form action="{{ route('finance.category.sub.edit', [$category->id, $sub->id]) }}" method="POST">

                @csrf

                <!-- Name Field -->
                <div class="mt-3">
                    <label class="font-medium">Category Name <span class="text-red-600 ml-1" aria-hidden="true">*</span>
                        <span class="sr-only">required</span></label>
                    <input type="text" name="name" class="input w-full border mt-2"
                        placeholder="e.g., Operational, Savings, Personal Expense" value="{{ $sub->name }}" required>
                </div>
