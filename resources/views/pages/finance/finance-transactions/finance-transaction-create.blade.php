...
<div class="grid grid-cols-12 mt-6">
    <div class="intro-y col-span-12 lg:col-span-10 lg:col-start-2">
        <div class="intro-y box p-6 rounded-xl shadow-sm border border-gray-100 bg-white">
            <form action="{{ route('finance.transaction.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mt-3">
                        @foreach ($financeAccounts as $item)
                            <div class="finance-account-card border-2 border-gray-200 rounded-xl p-3 cursor-pointer hover:border-theme-1 hover:shadow-md transition-all duration-150 text-center bg-white"
                                data-id="{{ $item->id }}">
                                @if (!empty($item->logo))
                                ...
                                @else
                                    <div
                                        class="mx-auto w-10 h-10 flex items-center justify-center bg-gray-100 rounded-lg text-sm font-semibold text-gray-600">
                                ...
                                @if (isset($item->balance))
                                    <p class="text-xs text-gray-500 mt-1">Rp
                                        {{ number_format($item->balance, 0, ',', '.') }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="date" class="input w-full border mt-2 rounded-lg text-sm"
                ...
                <div class="flex justify-end pt-4 border-t border-gray-100 mt-6">
                    <button type="button" onclick="history.back()"
                        class="button w-28 border text-gray-700 mr-2 hover:bg-gray-100 transition">Cancel</button>
                    <button type="submit"
                        class="button w-28 bg-theme-1 text-white hover:opacity-90 transition">Save</button>
                </div>
            </form>
        </div>
    </div>
